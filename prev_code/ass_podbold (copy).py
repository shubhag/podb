...
def searchforkey(key, value, filename):
	global tempCount
	global maxitem
	global root
	f = open(filename, 'r+')
	...
	if isleaf == '1':
		...
		if numKeys <= maxitem : 
			...
			newleaffile = 'podb' + str(tempCount)
			tempCount = tempCount + 1
			if parentfilename == '-1':
				parentfilename = 'podb' + str(tempCount)
				root = parentfilename
				tempCount = tempCount + 1
...
def recurBreak(leafl, leafr, key, filename ):
	...
	if numKeys <= maxitem:
		...
		siblingfile = 'podb' + str(tempCount)
		tempCount = tempCount + 1
		parentfilename = data[2].rstrip()
		if parentfilename != '-1':
			printToOpenFile(str(numKeys1) + '\n', data[1] ,data[2], ' '.join(keyvalue1) + '\n', ' '.join(ptrvalue1) + '\n', f)
